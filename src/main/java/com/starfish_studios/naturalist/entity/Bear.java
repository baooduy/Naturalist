...
public class Bear extends AnimalEntity implements Angerable, IAnimatable, SleepingAnimal {
    ...
    private static final Ingredient FOOD_ITEMS = Ingredient.fromTag(NaturalistTags.Items.BEAR_TEMPT_ITEMS);
    ...
    public PassiveEntity createChild(ServerWorld level, PassiveEntity mob) {
        return NaturalistEntityTypes.BEAR.create(level);
    }
    ...
    private void addEatingParticles() {
        if (this.getEatCounter() % 5 == 0 || this.getEatCounter() == 0) {
            this.playSound(NaturalistSoundEvents.BEAR_EAT, 0.5F + 0.5F * (float)this.random.nextInt(2), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
    ...
    public boolean damage(DamageSource pSource, float pAmount) {
        if (!this.getMainHandStack().isEmpty() && !this.world.isClient) {
            ...
            this.playSound(NaturalistSoundEvents.BEAR_SPIT, 1.0F, 1.0F);
    ...
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return NaturalistSoundEvents.BEAR_HURT;
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return NaturalistSoundEvents.BEAR_DEATH;
    }

    @Nullable
    @Override
    protected SoundEvent getAmbientSound() {
        return this.isSleeping() ? NaturalistSoundEvents.BEAR_SLEEP : this.isBaby() ? NaturalistSoundEvents.BEAR_AMBIENT_BABY : NaturalistSoundEvents.BEAR_AMBIENT;
    }
    ...
    static class BearHarvestFoodGoal extends MoveToTargetPosGoal {
        ...
        public void tick() {
            if (this.hasReached()) {
                ...
                bear.playSound(NaturalistSoundEvents.BEAR_SNIFF, 1.0F, 1.0F);
    ...
    static class BearPickupFoodAndSitGoal extends Goal {
        ...
        @Override
        public boolean canStart() {
            if (this.cooldown <= bear.age && !bear.isBaby() && !bear.touchingWater() && !bear.isSleeping() && !bear.isSitting()) {
