...
public class Snake extends ClimbingAnimal implements SleepingAnimal, Angerable, IAnimatable {
    ...
    private static final Ingredient FOOD_ITEMS = Ingredient.fromTag(NaturalistTags.Items.SNAKE_TEMPT_ITEMS);
    ...
    public EntityData initialize(ServerWorldAccess pLevel, LocalDifficulty pDifficulty, SpawnReason pReason, @Nullable EntityData pSpawnData, @Nullable NbtCompound pDataTag) {
        this.initEquipment(pDifficulty);
        return super.initialize(pLevel, pDifficulty, pReason, pSpawnData, pDataTag);
    }

    @Override
    protected void initEquipment(LocalDifficulty pDifficulty) {
    ...
    public void tickMovement() {
        ...
        if (this.canRattle() && !this.isSleeping()) {
            this.playSound(NaturalistSoundEvents.SNAKE_RATTLE, 0.15F, 1.0F);
        }
    }
    ...
    public boolean tryAttack(Entity pEntity) {
        if (this.getType().equals(NaturalistEntityTypes.CORAL_SNAKE) && pEntity instanceof LivingEntity living) {
    ...
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return NaturalistSoundEvents.SNAKE_HURT;
    }
    ...
    private void soundListener(SoundKeyframeEvent<Snake> event) {
        ...
        if (snake.world.isClient) {
            if (event.sound.equals("hiss")) {
                snake.world.playSound(snake.getX(), snake.getY(), snake.getZ(), NaturalistSoundEvents.SNAKE_HISS, snake.getSoundCategory(), snake.getSoundVolume(), snake.getSoundPitch(), false);
            }
        }
    }
