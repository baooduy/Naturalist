    ...
    private static class CocoonGoal extends MoveToTargetPosGoal {
        ...
        public void tick() {
            ...
            if (this.isTargetPos(level, caterpillar.getBlockPos())) {
                if (!level.isClient) {
                    ((ServerWorld)level).spawnParticles(new BlockStateParticleEffect(ParticleTypes.BLOCK, NaturalistBlocks.CHRYSALIS.getDefaultState()), caterpillar.getX(), caterpillar.getY(), caterpillar.getZ(), 50, caterpillar.getWidth() / 4.0F, caterpillar.getHeight() / 4.0F, caterpillar.getWidth() / 4.0F, 0.05D);
                }
                caterpillar.discard();
                level.setBlockState(caterpillar.getBlockPos(), NaturalistBlocks.CHRYSALIS.getDefaultState().with(ChrysalisBlock.FACING, facing));
