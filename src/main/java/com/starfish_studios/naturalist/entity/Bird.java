...
public class Bird extends TameableShoulderEntity implements Flutterer, IAnimatable {
    ...
    private static final Ingredient TAME_FOOD = Ingredient.ofItems(Items.WHEAT_SEEDS, Items.PUMPKIN_SEEDS, Items.BEETROOT_SEEDS, Items.MELON_SEEDS);
    public float flap;
    ...
    public ActionResult interactMob(PlayerEntity pPlayer, Hand pHand) {
        ...
        if (!this.isTamed() && TAME_FOOD.test(stack)) {
            ...
            if (!this.isSilent()) {
                this.world.playSound(null, this.getX(), this.getY(), this.getZ(), NaturalistSoundEvents.BIRD_EAT, this.getSoundCategory(), 1.0F, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.2F);
            }
        ...
        } else if (!this.isInAir() && this.isTamed() && this.isOwner(pPlayer)) {
            if (!this.world.isClient) {
    ...
    protected void addFlapEffects() {
        this.playSound(NaturalistSoundEvents.BIRD_FLY, 0.15F, 1.0F);
    ...
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return NaturalistSoundEvents.BIRD_HURT;
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return NaturalistSoundEvents.BIRD_DEATH;
    }
    ...
    protected SoundEvent getAmbientSound() {
        if (this.getType().equals(NaturalistEntityTypes.BLUEJAY)) {
            return NaturalistSoundEvents.BIRD_AMBIENT_BLUEJAY;
        } else if (this.getType().equals(NaturalistEntityTypes.CANARY)) {
            return NaturalistSoundEvents.BIRD_AMBIENT_CANARY;
        } else if (this.getType().equals(NaturalistEntityTypes.CARDINAL)) {
            return NaturalistSoundEvents.BIRD_AMBIENT_CARDINAL;
        } else {
            return NaturalistSoundEvents.BIRD_AMBIENT_ROBIN;
        }
    }
